...
/* The training overlay is intensity data, so under the three-palette rule
   it belongs on the blue depth ramp — not on the neutral paper-greys it
   used to borrow. The pale Easy end keeps it recessive: training is the
   background to the mentions, never a competing series. */
const PACE = "var(--color-pace-easy)";
...
/* Status is deliberately NOT a three-colour code.
   Coral is the alert palette, so an active niggle earns it — that is the
   one thing on this page the athlete might need to act on. Quiet and
   resolved are not alerts and do not get a hue of their own: they are
   separated by ink weight and by FORM (a resolved dot is hollow, a quiet
   thread trails off dashed). Green is mood-only under the three-palette
   rule, so the sage that used to mark "resolved" is gone — a resolved
   niggle is not a mood, and a "safe" state goes neutral, never green. */
const STATUS: Record<ThreadStatus, { label: string; color: string; note: string }> = {
  ...
  resolved: { label: "Resolved", color: INK_2, note: "athlete said it was gone" },
};
...
function Mono({
  ...
  return (
    <span
      className={`font-mono text-meta-sm tracking-meta uppercase ${className}`}
      style={{ color }}
    >
      {children}
    </span>
  ...
  return (
    <div className="bg-bg-card border border-divider rounded-lg px-5 py-4">
      ...
      <div className="mt-3 flex items-baseline gap-2">
        <span
          className="font-display text-display-xl leading-none tabular-nums"
      ...
      <p className="mt-2 font-body text-body-sm leading-[1.45] text-text-secondary">
        {secondary}
      </p>
    </div>
  );
...
function Section({
  ...
  return (
    <section className="mt-14">
      ...
      <h2 className="mt-2 font-display text-display-m sm:text-display-l leading-[1.1]">
        {title}
      </h2>
      <p className="mt-2 max-w-[640px] font-body italic text-body-sm leading-[1.5] text-text-secondary">
        {caption}
      </p>
...
/* ── Expandable summary card — the shipping container ─────────────── */

/* `outputs/ui-redesign/trends.md` restructures Trends into an
   `Effort · Fitness · Signal` sub-nav in which every deep metric is a
   COLLAPSED summary card — "one 44pt row (label · number · delta ·
   mini-sparkline · ▸)" — that expands *in place* to the full chart. The
   niggle timeline is one card inside **Signal**, not a page of its own,
   so that is the container it gets here.

   Two constraints from the brief are load-bearing:
     · the expanded state is NOT a card inside a card (hence Timeline's
       `framed={false}`), and
     · the collapsed row is a real 44pt tap target.

   The study below mounts it open so the whole design still reads on one
   scroll; in Trends it mounts closed, stacked with the mood arc and the
   felt-vs-measured cross-link that make up the rest of Signal. */

/** Mentions per week across the block — the collapsed row's preview. */
function Sparkline({ counts }: { counts: number[] }) {
  ...
  return (
    <svg
      width={counts.length * (bar + gap)}
      height={16}
      viewBox={`0 0 ${counts.length * (bar + gap)} 16`}
      className="shrink-0 hidden sm:block"
      aria-hidden
    >
      {counts.map((c, i) => {
        const h = c === 0 ? 1.5 : Math.max(3, (c / max) * 15);
        return (
          <rect
            key={i}
            x={i * (bar + gap)}
            ...
            width={bar}
            height={h}
            fill={c === 0 ? "var(--color-divider)" : INK_3}
          />
        );
      })}
    </svg>
...
function SummaryCard({
  ...
  return (
    <div className="bg-bg-card border border-divider rounded-lg">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="w-full min-h-[44px] px-5 py-3 flex items-center gap-3 sm:gap-4 text-left"
      >
        <span className="min-w-0 flex-1 flex flex-col sm:flex-row sm:items-baseline sm:gap-3">
          <span className="font-mono text-meta-sm tracking-label uppercase" style={{ color: INK_1 }}>
            {eyebrow}
          </span>
          <span className="font-body text-body-sm text-text-secondary truncate">{value}</span>
        </span>
        {delta && (
          <span
            className="font-mono text-meta-sm tracking-caption uppercase shrink-0 tabular-nums"
        ...
        {spark}
        <span className="font-mono text-meta-sm shrink-0" style={{ color: INK_3 }} aria-hidden>
          {open ? "▾" : "▸"}
        </span>
      </button>
      {open && <div className="px-5 pb-5 border-t border-divider-soft pt-4">{children}</div>}
    </div>
  );
}
...
function Timeline({
  ...
  /** Drop the card chrome when this renders inside a SummaryCard —
      `trends.md` is explicit that an expanded card is not a card-on-card. */
  framed?: boolean;
}) {
  ...
  return (
    <div
      className={
        framed
          ? "bg-bg-card border border-divider rounded-lg overflow-hidden"
    ...
    >
        ...
        <div className="flex">
          ...
          <div className={`hidden sm:block w-[168px] shrink-0 ${PLOT_H_CLS}`}>
            {threads.map((t) => {
              ...
              return (
                ...
                >
                  <span
                    className="font-mono text-meta-sm tracking-label uppercase"
                    style={{ color: INK_1, fontWeight: isSel ? 600 : 400 }}
                  >
                    {t.label}
                  </span>
                  <span
                    className="font-mono text-meta-sm tracking-caption uppercase"
                    style={{ color: INK_3 }}
                  >
                    {rowSummary(t)}
                  </span>
          ...
          >
                  ...
                  <div key={w.start} className="flex-1 h-full flex items-end px-[1.5px]">
                    <div
                      ...
                      style={{
                        ...
                        background: wash(PACE, w.partial ? 16 : 34),
                      }}
              ...
              return (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => onSelectThread(isSel ? null : t.key)}
                    className="sm:hidden absolute top-0 left-0 right-0 text-left font-mono text-meta-sm tracking-caption uppercase truncate"
                    style={{ color: INK_1, fontWeight: isSel ? 600 : 400 }}
                  >
                      ...
                      >
                        <span
                          ...
                          style={{
                            ...
                            border: `2px solid ${resolved ? INK_2 : PAPER}`,
                            boxShadow: isDotSel ? `0 0 0 3px ${wash(INK_1, 18)}` : "none",
                          }}
                        />
            ...
            <div
              ...
              style={{ left: `${xPct(TODAY)}%`, background: INK_3, opacity: 0.55 }}
              aria-hidden
            />
          </div>
        </div>
          ...
          <div className="relative flex-1 h-4">
            {axisTicks.map((w, i) => (
              <span
                ...
                className={`absolute font-mono text-meta-sm tracking-caption uppercase ${
            ...
            <span
              className="absolute right-0 font-mono text-meta-sm tracking-caption uppercase"
              style={{ color: INK_3 }}
            >
              Today
            </span>
      ...
      <div className="px-5 py-3 border-t border-divider-soft bg-bg-elevated flex flex-wrap items-center gap-x-6 gap-y-2">
        ...
        <LegendDot color={INK_2} label="Resolved · athlete&rsquo;s own words" hollow />
        <span className="inline-flex items-center gap-2">
          <span className="w-5 h-2 rounded-[1px]" style={{ background: wash(PACE, 34) }} />
...
function MentionReadout({ mention, thread }: { mention: BodyMention; thread: NiggleThread }) {
  ...
  return (
    <div className="mt-4 bg-bg-card border-l-2 rounded-r-lg px-5 py-4" style={{ borderColor: CORAL }}>
      ...
      <p className="mt-3 font-body italic text-body leading-[1.45] text-text-primary">
...
function ReadoutCell({ eyebrow, value }: { eyebrow: string; value: string }) {
  return (
    <div>
      <Mono>{eyebrow}</Mono>
      <p className="mt-1.5 font-body text-body-sm leading-[1.45] text-text-primary">{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="bg-bg-card border border-divider rounded-lg overflow-hidden">
      <div className="px-5 py-4 border-b border-divider-soft flex items-baseline justify-between gap-3">
        <span className="font-display text-display-s leading-none">
          {thread.label}
        </span>
        <span
          className="font-mono text-meta-sm tracking-label uppercase px-2 py-1 rounded-[2px]"
      ...
      <div className="px-5 py-4 border-b border-divider-soft">
        ...
        {thread.returns.length === 0 ? (
          <p className="mt-2 font-body text-body-sm leading-[1.5] text-text-secondary">
          ...
          <ul className="mt-2 space-y-1.5">
            {thread.returns.map((r) => (
              <li
                key={r.on}
                className="font-body text-body-sm leading-[1.5] text-text-primary"
              >
                Returned {shortDate(r.on)}, after{" "}
                <span className="tabular-nums" style={{ color: INK_1 }}>
                  {r.after} days
                </span>{" "}
                quiet.
              </li>
      ...
      {/* resolution */}
      {thread.resolution && (
        <div className="px-5 py-4 border-b border-divider-soft" style={{ background: wash(INK_2, 4) }}>
          <Mono color={INK_2}>Resolved · {shortDate(thread.resolution.mentionedAt)}</Mono>
          <p className="mt-2 font-body italic text-body leading-[1.45] text-text-primary">
      ...
      <div className="px-5 py-4">
          ...
          {rows.map((m) => {
            ...
            return (
              <li key={m.id}>
                ...
                <p className="mt-1 font-body italic text-body leading-[1.45] text-text-primary">
                  &ldquo;{m.quote}&rdquo;
                </p>
                <p className="mt-1 font-mono text-meta-sm tracking-caption uppercase" style={{ color: INK_3 }}>
        ...
        {thread.mentionCount > rows.length && (
          <p className="mt-4 font-mono text-meta-sm tracking-caption uppercase" style={{ color: INK_3 }}>
...
function Stat({ eyebrow, value }: { eyebrow: string; value: string }) {
  return (
    <div>
      <Mono>{eyebrow}</Mono>
      <p className="mt-1 font-body text-body leading-none text-text-primary tabular-nums">
        {value}
      </p>
    </div>
  );
    ...
    <ul className="space-y-2.5">
      {rows.map((r) => (
        <li key={r.label} className="grid grid-cols-[84px_1fr_24px] sm:grid-cols-[104px_1fr_28px] items-center gap-2 sm:gap-3">
          ...
          <span
            className="font-mono text-meta tabular-nums text-right"
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      <div className="bg-bg-card border border-divider rounded-lg px-5 py-5 lg:col-span-1">
        ...
        <p className="mt-4 font-display text-display-l sm:text-display-xl leading-none tabular-nums" style={{ color: CORAL }}>
          {longCount}
          <span className="text-text-tertiary"> / {mentions.length}</span>
        </p>
        <p className="mt-3 font-body text-body leading-[1.5] text-text-primary">
          mentions came within 24 hours of a long run.
        </p>
        <p className="mt-4 pt-3 border-t border-divider-soft font-body text-body-sm leading-[1.5] text-text-secondary tabular-nums">
      ...
      <div className="bg-bg-card border border-divider rounded-lg px-5 py-5">
        ...
        <div className="mt-4">
          <TallyBars rows={tallyBySession(mentions)} accent={PACE} />
        </div>
      </div>
...
export default function NigglesDashboard() {
  ...
  /* The collapsed row's preview: one bar per week of the block, counting
     mentions (resolutions excluded — they are the thread ending, not
     another instance of it). */
  const weeklyMentionCounts = useMemo(
    () =>
      WEEKS.map(
        (w) => ALL_MENTIONS.filter((m) => weekOf(m.mentionedAt)?.start === w.start).length
      ),
    []
  );
    ...
    <div className="min-h-screen bg-bg-base text-text-primary font-body">
      ...
      <main className="mx-auto max-w-[1080px] px-5 sm:px-10 py-10 sm:py-14">
        ...
        <section>
          ...
          <h1 className="mt-3 font-display text-display-xl sm:text-display-2xl leading-[1.0]">
            Niggles.
          </h1>
          <p className="mt-5 max-w-[660px] font-body text-body leading-[1.6] text-text-secondary">
          ...
          <p className="mt-4 font-mono text-meta-sm tracking-label uppercase" style={{ color: INK_3 }}>
        ...
        <Section
          ...
          caption="One row per body area. Each dot is a voice log. Pick an overlay to put your training behind the dots; tap a dot to read what you actually said that day. This is one card inside Trends → Signal: it ships collapsed to the row above, and opens in place."
        >
          <SummaryCard
            eyebrow="Niggle timeline"
            value={`${THREADS.length} body areas · ${ALL_MENTIONS.length} mentions · ${blockWeeks} weeks`}
            delta={activeCount > 0 ? `${activeCount} active` : "None active"}
            deltaColor={activeCount > 0 ? CORAL : INK_2}
            spark={<Sparkline counts={weeklyMentionCounts} />}
            defaultOpen
          >
          {/* overlay switcher */}
          <div className="mb-4 flex flex-wrap items-center gap-2">
            ...
            {OVERLAYS.map((o) => {
              ...
              return (
                <button
                  ...
                  className="font-mono text-meta-sm tracking-label uppercase px-2.5 sm:px-3 py-1.5 rounded-[3px] border transition-colors"
                  style={{
                    color: on ? PAPER : INK_2,
                    background: on ? INK_1 : "transparent",
                    borderColor: on ? INK_1 : "var(--color-divider)",
                  }}
                >
                  {o.label}
                </button>
            ...
            {selectedKey && (
              <button
                ...
                className="ml-auto font-mono text-meta-sm tracking-label uppercase underline underline-offset-4"
                style={{ color: INK_2 }}
              >
                Clear filter ↩
              </button>
          ...
          {selectedMention && mentionThread ? (
            ...
            <p className="mt-4 font-body italic text-body-sm leading-[1.5] text-text-secondary">
        ...
        >
          ...
          <p className="mt-5 font-body italic text-body-sm leading-[1.5] text-text-secondary">
            {selectedThread
          ...
          <ul className="mt-4 space-y-2.5">
            {(["active", "quiet", "resolved"] as ThreadStatus[]).map((s) => (
              <li key={s} className="flex items-baseline gap-3">
                <span
                  className="font-mono text-meta-sm tracking-label uppercase px-2 py-1 rounded-[2px] shrink-0"
                  style={{ color: STATUS[s].color, background: wash(STATUS[s].color, 12) }}
                >
                  {STATUS[s].label}
                </span>
                <span className="font-body text-body-sm leading-[1.5] text-text-secondary">
                  {STATUS[s].note}
                </span>
              </li>
            ))}
          </ul>
          <p className="mt-4 pt-3 border-t border-divider-soft font-body italic text-body-sm leading-[1.5] text-text-secondary">
        ...
        <footer className="mt-12 pt-6 border-t border-divider-soft">
          <p className="font-body italic text-body-sm leading-[1.5] text-text-secondary">
          ...
          <div className="mt-5 flex items-center justify-between gap-4">
            ...
            <Link
              href="/design"
              className="font-mono text-meta-sm tracking-meta uppercase text-text-primary hover:text-coral transition-colors"
            >
              Back to previews ↗
            </Link>
          </div>
