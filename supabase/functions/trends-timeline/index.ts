...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // The lap-level replacement for the surface above: every lap the watch
    // recorded, positioned by percent of goal pace, never merged and never
    // averaged. See goalPaceGrid.ts for why the parser's blocks were dropped.
...
async function buildGoalPaceGridBlock(
  ...
  // The watch's laps come off the same `logs` rows the timeline already
  // loaded (`LOG_COLS_BASE` selects `pace_segments`) — no second fetch, and
  // no `parsed_structure`: this surface reads the tape, not the parser.
  return buildGoalPaceGrid(
    logs.map((l) => ({
      id: l.id,
      workout_date: l.workout_date,
      workout_type: l.workout_type ?? null,
      workout_distance_miles: l.workout_distance_miles ?? null,
      workout_duration_minutes: l.workout_duration_minutes ?? null,
      pace_segments: l.pace_segments ?? null,
    })),
    {
      race_key: raceKey,
