...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const prompt = loadPrompt("biomechanics-analysis.v1", {
      viewAngle: analysis.view_angle,
      multiAngleNote: analysis.notes?.startsWith("Combined from")
        ? `\n- Multi-angle capture: ${analysis.notes}`
        : "",
      durationSeconds: analysis.duration_seconds?.toFixed(1) ?? "",
      frameCount: analysis.frame_count ?? "",
      fps: analysis.fps?.toFixed(0) ?? "",
      jointLines: jointLines.filter((l) => l).join("\n") || "No joint angle data available",
      footStrikeBlock: formatFootStrike(footStrike),
      gaitMetricsBlock: formatGaitMetrics(gaitMetrics),
