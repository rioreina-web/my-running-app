 ...
 * `extracted_data`.
 *
 * ── What changed v1 → v2 (2026-07-02, niggle-classifier rewrite) ──
 * The `soreness` field graduates from a flat string array
 * (`["left knee", "calves"]`) to structured objects:
 *   [{ "location": "left knee",
 *      "their_words": "knee felt cranky after mile 8",
 *      "severity_word": "cranky" }]
 * The memo pipeline is now the primary niggle CLASSIFIER + WRITER: the
 * function runs each `location` through the closed body-part vocabulary
 * (`_shared/bodyVocabulary.ts`) and persists valid mentions to
 * `body_mentions` with laterality, the athlete's verbatim words, and a
 * closed severity hint. So this prompt must emit the athlete's own
 * location and phrasing — never a diagnosis or condition name. The
 * closed vocabulary is enforced in CODE after this returns; the prompt's
 * job is faithful capture, not classification.
 *
 * Two wedge-relevant constraints this prompt must preserve (hard rule #2):
 *   - `mood` stays within the closed vocabulary
 *     (energized | positive | neutral | tired | struggling | injured)
 *   - `coach_insight` and `soreness` never make medical diagnoses and
 *     never recommend stop-training language outside the documented
 *     exceptions. Location is anatomy, never a condition.
 *
 * The audio transcript and any Garmin watch data are appended to the
 * rendered prompt by the caller, NOT substituted into the template.
 *
