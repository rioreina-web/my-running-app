...
- For coach_insight: when pace zones are available above, reference the athlete's REAL bands and anchors. Don't invent pace numbers. When the zone classification line shows the athlete ran outside their expected band for the workout type (e.g. an "easy" run that landed in moderate), call that out directly. When a "Workout splits" block appears, comment on the shape ONLY when it carries a "Pattern:" line — pick that meaningful read rather than reciting every rep. If there's no Pattern line (an easy/steady/long run where drift is normal), do NOT invent a fade or a shape; the splits are just context. When a "Workout progression" block appears showing real movement vs. a comparable prior session — longer, faster, or both — that's the headline of your read; say so plainly.`;
