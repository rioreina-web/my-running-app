/**
 * Voice memo analyzer prompt — v6 (drops the verbatim `transcription` echo
 * from the output contract. The caller already holds the transcript — it is
 * the input — so asking the model to type it back cost ~200-450 output
 * tokens per memo and ~0.7s of a ~2.4s analysis call (measured 2026-09-04,
 * thinking off: 2.3-2.6s with the echo, 1.6-1.9s without). Otherwise
 * identical to v5.)
 *
 * v5 history: adds `no_niggles`, the explicit global
 * all-clear: "legs feel fine", "nothing hurts". Feeds a severity 'none'
 * body_mentions row so the daily_scores soreness baseline includes zero
 * days — step 6 of the drip-scores handoff. Otherwise identical to v4.)
 *
 * Consumed by `supabase/functions/process-training-memo/index.ts`. v4 was
 * v3 minus the `coach_insight` field, its CRITICAL RULES section, and the
...
3. **workout_notes**: A structured text summary of quantitative training details mentioned. Use this format with one item per line:
...
4. **extracted_data**: A JSON object with structured numeric/typed data extracted from the memo. Only include fields that were mentioned:
   {
     ...
     "no_niggles": {"their_words": "legs felt great, nothing hurts"} or null (the GLOBAL all-clear — see rules below),
   ...
   RULES FOR no_niggles (the global "nothing hurts" signal):
   - Set it ONLY when the athlete EXPLICITLY says their body as a whole is fine — "legs feel great", "nothing hurts", "no niggles", "body felt good". Their verbatim phrasing goes in their_words.
   - Silence is NOT an all-clear. A memo that simply never mentions the body gets no_niggles = null. Never infer it from a good mood, a fast pace, or the absence of complaints.
   - If ANY body part appears in soreness in this memo, no_niggles must be null — a specific complaint beats a general "felt fine".

5. **memory_candidates**: An array of durable things worth remembering about this athlete AS A PERSON — facts, stable preferences, structural constraints, life context, gear, and genuinely distinctive moments — drawn ONLY from what they actually said. This is the coach's long-term memory: a good coach remembers ~30 things about an athlete, not everything. MOST memos contain nothing memorable. Return [] and that is the correct, common answer — do NOT manufacture a memory from a routine run.
