...
> - **Third pass, 2026-06-11:** blocked daily-reads migration pair quarantined; 3 apply-ready migrations re-stamped to 2026-06-11 (incl. backfill_via_outbox — verified NEEDED: 133 candidates, empty outbox); secrets inventory written (docs/deploy/secrets-inventory.md — note: ANTHROPIC_API_KEY has zero consumers left, revoke if set); CLAUDE.md refreshed (hard rule #9: db push only; Task #23 synthesis trigger verified RESOLVED in prod); record-evals.yml one-click cassette recording (2.4 unblocked — 4/21 recorded, 17 stubs likely recordable as-is); preview-branch setup documented (docs/deploy/preview-branches.md — use the official GitHub integration; $0.013/hr/branch; requires paid plan, org currently FREE).
