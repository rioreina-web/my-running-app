...
const InjuryRow = ({ name, side, days, mentions, miles, load, trend, trendClass, dots, lastQuote, firstLine, onOpen }) => (
  <div className="injury" onClick={onOpen} style={{ cursor: onOpen ? "pointer" : "default" }}>
    ...
    <div style={{ display: "flex", gap: 16, marginTop: 4 }}>
      <a className="link" style={{ fontSize: 12 }} onClick={(e) => { e.stopPropagation(); onOpen && onOpen(); }}>View detail</a>
      <a className="link" style={{ fontSize: 12, color: "var(--ink-2)", borderColor: "var(--rule)" }} onClick={(e) => { e.stopPropagation(); onOpen && onOpen(); }}>Update</a>
      <a className="link" style={{ fontSize: 12, color: "var(--ink-2)", borderColor: "var(--rule)" }} onClick={(e) => e.stopPropagation()}>Mark resolved</a>
    </div>
  </div>
);

const InjuriesScreen = ({ onClose, onAdd, onOpenInjury }) => (
  <div className="page">
    ...
    <div className="page__body">
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <a className="link" onClick={onAdd} style={{ fontSize: 13, cursor: "pointer" }}>+ Add</a>
      </div>
