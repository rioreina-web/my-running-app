...
//  RunningLog
//
//  Direction A · "Editorial" port of the Log Details sheet.
//
//  This file is a *drop-in replacement* for the body of HistoryDetailSheet.
//  Diff against the existing file rather than copying wholesale — the
//  toolbar wiring, sheet presentation, alert, and `.task`/`.onAppear` hooks
//  at the bottom of the original file all stay as they are. Only the
//  `ScrollView { VStack { … } }` block in `body` changes.
//
//  Depends on:
...
//    • Existing vm + state from HistoryDetailSheet
//
...
extension HistoryDetailSheet {
    ...
    // ────────────────────────────────────────────────────────────────────
    //
    // Paste this in place of the current `ScrollView { VStack(spacing: 24) { … } }`
    // block in `HistoryDetailSheet.body`. The wrapping `NavigationStack` +
    // `ZStack` + `Color.drip.background.ignoresSafeArea()` stay.
    //
    @ViewBuilder
        ...
        ScrollView {
            VStack(spacing: 0) {
                ...
                DripPlateStrip(
                    ...
                    trailingTop: vm.currentEntry.displayDate.shortDateString,
                    trailingBottom: vm.currentEntry.displayDate.shortTimeString
                )
                        ...
                        if let insight = vm.coachInsight, !insight.isEmpty {
                            ...
                            DripTextLink(title: "Ask the coach →") {
                                Task {
                                    ...
                                    // call existing CoachInsightSection generator path
                                    // (factor into vm.generateCoachInsight if not already)
                                }
                            }
                            .padding(.top, 8)
                        }
    ...
    // ────────────────────────────────────────────────────────────────────
    // Stat strip data — pulls from the linked workout if present, else
    // falls back to the raw notes-derived fields the user typed.
    // ────────────────────────────────────────────────────────────────────
    private var editorialStats: [DripStat]? {
        ...
        if let hr = vm.currentEntry.workoutAverageHeartRate {
            stats.append(DripStat("HR", "\(Int(hr))", unit: "bpm"))
        }
        if let elev = vm.currentEntry.workoutElevationGainMeters {
            stats.append(DripStat("ELEV", "\(Int(elev))", unit: "m"))
        }
