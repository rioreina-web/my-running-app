/* global React */
/* ════════════════════════════════════════════════════════════════════
   POST RUN DRIP — TRAINING SUMMARY (v1)
...
// Day-by-day for the current week (Mon-first). nulls = no run.
const WEEK_DAYS = [
  ...
  { day: "SAT", date: "May 16", miles: 0,    pace: null,   mood: null,        type: "planned"  }, // ahead of today
...
const WEEK_TOTALS = (() => {
  ...
  const planned = 51;
  // weighted average pace by miles
  const totalMin = done.reduce(
    (s, d) => s + (paceToMin(d.pace) || 0) * d.miles, 0
  );
  const avgPace = miles > 0 ? totalMin / miles : 0;
  const moods = done.map((d) => d.mood).filter(Boolean);
  const moodCounts = moods.reduce((acc, m) => ((acc[m] = (acc[m]||0)+1), acc), {});
...
// Prior 3 weeks for the trajectory chart (4 total)
const FOUR_WEEKS = [
...
// Mood heatmap data — 4 weeks × 7 days
// row 0 = oldest week, row 3 = current
const MOOD_GRID = [
  ...
  [null,"positive","positive","neutral","energized",null,null], // current week, future = empty
];
...
// Niggles / injuries surfaced from voice logs (this week)
const NIGGLES = [
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        ...
        <div className="mt-6">
          ...
          <ul className="mt-2 space-y-0.5">
            {more.map((it) => (
              <li key={it.label}>
                <a href={it.href} className="block px-3 py-1.5 rounded-md text-[13px] text-text-secondary hover:text-text-primary hover:bg-bg-elevated">
                  {it.label}
                </a>
              </li>
            ))}
...
function TopNav() {
  return (
    <header className="bg-bg-base border-b border-divider px-8 py-3 flex items-center justify-between">
      <Mono>RUNNING LOG · DASHBOARD</Mono>
...
function Lede() {
  ...
  return (
    <section>
      <Mono color={ACCENT}>THIS WEEK · {WEEK_LABEL.start} – {WEEK_LABEL.end}</Mono>
      <h1 className="mt-3 font-display text-[64px] leading-[0.98] tracking-[-0.02em] max-w-[840px]">
        ...
        <em className="font-display italic text-text-secondary">
          averaging <span style={{ color: ACCENT }} className="tabular-nums">{t.avgPaceLabel}</span> a mile.
        </em>
      </h1>
      <p className="mt-5 max-w-[640px] font-body text-[16px] leading-[1.6] text-text-secondary">
        Halfway through the week and the build is holding. Tuesday&rsquo;s tempo
        was the centerpiece; Sunday&rsquo;s 18 is the test that follows.
      </p>
    </section>
  );
}
...
function StatsGrid() {
  ...
  return (
    <section className="grid grid-cols-2 md:grid-cols-4 gap-0 border-y border-divider divide-x divide-divider">
      ...
      <StatCell
        label="MOOD"
        value={MOOD[t.topMood].label}
        hint="across the run days"
      >
...
function StatCell({ label, value, unit, hint, accent, trend, children }) {
  return (
...
function MileagePctBar({ pct, accent }) {
  return (
...
function RunsDots({ done, planned }) {
  return (
...
function DailyPaceLine({ days, accent }) {
  // tiny dot-plot — one dot per day with a pace
    ...
    <div className="relative h-6">
      ...
      <div className="absolute inset-0 flex items-center justify-between">
        {paces.map((p, i) => {
          ...
          const y = ((p - min) / range) * 16; // shift up to 16px
          return (
            <span
              key={i}
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 h-[64px] flex items-end">
                {d.miles > 0 ? (
                  <div
                    ...
                    style={{
                      height: `${h}px`,
                      background: d.type === "tempo" || d.type === "intervals" ? ACCENT :
                                  d.type === "long" ? SAGE :
                                  "#1A1815",
                      opacity: 0.85,
                    }}
              ...
              <p className="mt-2 font-mono tabular-nums text-[12px] text-text-primary">
                ...
                {d.miles > 0 ? (
                  <span className="ml-0.5 text-[9px] tracking-[1.3px] uppercase text-text-tertiary">mi</span>
                ) : null}
              </p>
              ...
              {isPast && d.miles > 0 ? (
                ...
                <p className="mt-1 font-mono text-[9px] tracking-[1.2px] uppercase" style={{ color: ACCENT }}>
                  today
                </p>
              ) : (
                <p className="mt-1 font-mono text-[9px] tracking-[1.2px] uppercase text-text-tertiary">
...
function TrajectoryChart({ weeks, max, accent }) {
  const W = 480;
  const H = 180;
  const padL = 40;
  const padR = 12;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto block">
      {yTicks.map((t) => {
        ...
        return (
          <g key={t}>
            <line x1={padL} x2={W - padR} y1={y} y2={y} stroke="#E8E4E0" strokeDasharray={t === 0 ? "0" : "2 3"} strokeWidth="1" />
            <text x={padL - 8} y={y + 3} textAnchor="end" fontFamily="ui-monospace, Menlo, monospace" fontSize="9" letterSpacing="1.2" fill="#9B9590">
              {t}
            </text>
          </g>
        );
        ...
        return (
          <g key={i}>
            ...
            <rect
              ...
              rx="1"
            />
            <text x={x + barW / 2} y={H - 18} textAnchor="middle" fontFamily="ui-monospace, Menlo, monospace" fontSize="9.5" letterSpacing="1.2" fill={w.current ? accent : "#1A1815"} fontWeight={w.current ? 700 : 500}>
              {w.miles.toFixed(0)}
            </text>
            <text x={x + barW / 2} y={H - 6} textAnchor="middle" fontFamily="ui-monospace, Menlo, monospace" fontSize="8.5" letterSpacing="1.2" fill="#9B9590">
              {w.label}
            </text>
          </g>
        );
      ...
      <div className="mt-4 grid grid-cols-[20px_1fr] gap-x-3">
        ...
        <div className="flex flex-col justify-between py-1">
          {["Apr 20","Apr 27","May 4","This wk"].map((wk, i) => (
            <span key={i} className="font-mono text-[8.5px] tracking-[1.2px] uppercase text-text-tertiary leading-none whitespace-nowrap">
              {wk}
            </span>
          ))}
        </div>
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-7 gap-[4px]">
            {days.map((d, i) => (
              <span key={i} className="text-center font-mono text-[8.5px] tracking-[1px] uppercase text-text-tertiary">
                {d}
              </span>
            ))}
          </div>
...
function RunRow({ run, isLast }) {
  const typeColor =
    run.type === "tempo" || run.type === "intervals" ? ACCENT :
    run.type === "long" ? SAGE :
    run.type === "race" ? "#B83A4A" :
    "#9B9590";
  ...
  return (
    ...
    >
      ...
      <span className="font-mono text-[10.5px] tracking-[1.4px] uppercase" style={{ color: typeColor, fontWeight: 600 }}>
        {run.type}
      </span>
