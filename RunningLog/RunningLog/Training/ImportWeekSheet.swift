...
struct ImportWeekSheet: View {
    ...
    private var totalWeeks: Int {
        viewModel.activePlan?.totalWeeks ?? 16
    }
        ...
        NavigationStack {
            ZStack {
                ...
                if applied {
                    // Success state
                    successView
                } else if showPreview, let days = importService.importedWorkouts {
                    // Preview state
            ...
            .onAppear {
                selectedWeek = initialWeekNumber
            }
            .onChange(of: selectedWeek) { _, newValue in
                viewModel.selectedWeek = newValue
            }
        }
    }
    ...
    private var inputView: some View {
        VStack(spacing: 20) {
            ...
            Button {
                ...
                Task {
                    await importService.parseWeekFromText(inputText)
                    if importService.importedWorkouts != nil {
                        withAnimation(.spring(response: 0.3)) {
                            showPreview = true
                        }
                    }
                ...
                HStack(spacing: 10) {
                    if importService.isParsingImport {
                        ProgressView()
                            .tint(.white)
                    } else {
                    ...
                    Text(importService.isParsingImport ? "Extracting..." : "Extract Workouts")
                        .font(.dripLabel(16))
                }
                ...
                .padding(.vertical, 16)
                .background(
                    inputText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || importService.isParsingImport
                        ? Color.drip.textTertiary
                        : Color.drip.coral
                )
                .clipShape(RoundedRectangle(cornerRadius: 14))
            }
            .disabled(inputText.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || importService.isParsingImport)
            .padding(.horizontal, 20)
            .padding(.bottom, 20)
        }
    }
    ...
    private func previewView(days: [ImportedDayWorkout]) -> some View {
        VStack(spacing: 0) {
            ...
            VStack {
                Button {
                    Task {
                        await importService.applyImportedWorkouts()
                        withAnimation(.spring(response: 0.3)) {
                            applied = true
                        }
                    ...
                    HStack(spacing: 10) {
                        ...
                        Text(viewModel.isSaving ? "Applying..." : "Apply to Week \(selectedWeek)")
                            .font(.dripLabel(16))
                    }
                    .foregroundStyle(.white)
                    .frame(maxWidth: .infinity)
...
struct ImportDayPreviewRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            // Day indicator — hide for secondary sessions (doubles)
            VStack(spacing: 2) {
                if !day.isSecondarySession {
                    Text(String(day.dayName.prefix(3)).uppercased())
                        .font(.dripCaption(9))
                        .foregroundStyle(Color.drip.textTertiary)
                } else {
                    // Show a connector line for doubles
                    Text("+")
                        .font(.dripCaption(9))
                        .foregroundStyle(Color.drip.textTertiary)
                }
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(day.isSecondarySession ? workoutType.color.opacity(0.2) : Color.drip.divider, lineWidth: 1)
        )
    }
}
...
#Preview {
    ...
    ImportWeekSheet(
        viewModel: vm,
        importService: vm.importService,
        initialWeekNumber: 5
    )
}
