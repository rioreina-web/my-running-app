...
//  RunningLog
//
//  Main view for the AI workout generator.
//
...
struct WorkoutGeneratorView: View {
    ...
    @State private var selectedWorkoutForDetail: PlannedWorkout?
...
struct PhaseProgressBar: View {
    ...
    private let phases = TrainingPhase.allCases
...
struct SignatureWorkoutGrid: View {
    ...
    let onSelect: (SignatureType) -> Void
...
struct CategoryButtonRow: View {
    let isGenerating: Bool
    let onSelect: (PlannedWorkoutCategory) -> Void

    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 12) {
                ForEach(PlannedWorkoutCategory.allCases, id: \.self) { category in
                    CategoryButton(
                        category: category,
                        isLoading: isGenerating
                    ) {
