            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                if let intensity = step.targetPaceIntensity {
                    HStack(spacing: 8) {
                        ...
                        // Priority 1: "in M:SS-M:SS" for distance-based steps with actual pace data
                        if let targetTime = intensity.formattedTargetTime(
                            forDistance: step.durationValue, durationType: step.durationType
                        ) {
                            Text(targetTime)
                                .font(.dripLabel(13))
                                .foregroundStyle(Color.drip.energized)
                        // Priority 2: actual pace/km when we have real pace data
                        } else if intensity.paceSecondsPerKm != nil {
                            Text(intensity.formattedPace(forRacePace: racePaceSeconds))
                                .font(.dripLabel(13))
                                .foregroundStyle(Color.drip.energized)
                        }
                        // No actual pace data → show named pace as primary display
                        // (avoids absurd computed values like "14:05/mi" from percentage math)
                        ...
                        if let equiv = equivalentPaces,
                           let namedPace = equiv.closestNamedPace(
                            ...
                            Text(namedPace.shortName)
                                .font(intensity.paceSecondsPerKm != nil ? .dripCaption(11) : .dripLabel(13))
                                .foregroundStyle(namedPace.color)
                                .padding(.horizontal, intensity.paceSecondsPerKm != nil ? 6 : 8)
                                .padding(.vertical, intensity.paceSecondsPerKm != nil ? 2 : 4)
                                .background(namedPace.color.opacity(0.15))
                                .clipShape(Capsule())
                        } else if intensity.paceSecondsPerKm == nil {
                            // No actual pace and no named match — show percentage
                            Text("\(intensity.displayPercentage) effort")
                                .font(.dripLabel(13))
                                .foregroundStyle(Color.drip.energized)
                        } else {
