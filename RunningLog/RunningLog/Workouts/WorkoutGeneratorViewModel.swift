...
//  RunningLog
//
//  View model for the AI workout generator.
//
...
final class WorkoutGeneratorViewModel {
    ...
    var generatedWorkouts: [PlannedWorkout] = []
    var selectedWorkout: PlannedWorkout?
    ...
    private func calculatePhase(for goal: UserGoal) {
        ...
        currentPhase = TrainingPhase.fromWeeksOut(weeksUntilRace)
    }
    ...
    @MainActor
    func generateWorkout(category: PlannedWorkoutCategory) async {
        ...
        do {
            ...
            var token = supabaseAnonKey
            if let session = try? await supabase.auth.session {
                token = session.accessToken
            } else if let refreshed = try? await supabase.auth.refreshSession() {
    ...
    @MainActor
    func generateQuickWorkout(signatureType: SignatureType) {
        guard let goalTime = marathonGoalTime else {
            errorMessage = "Please set a goal time first"
            showError = true
            return
