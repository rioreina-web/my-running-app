...
//  RunningLog
//
//  Service for exporting Canova workouts to Garmin FIT file format.
//
...
final class FITExportService {
    ...
    /// Exports a CanovaWorkout to a FIT file and returns the file URL
    /// - Parameters:
    ...
    func exportWorkout(_ workout: CanovaWorkout, racePaceSeconds: Double) async throws -> URL {
    ...
    private func generateFITData(for workout: CanovaWorkout, racePaceSeconds: Double) throws -> Data {
        var data = Data()
    ...
    private func createWorkoutMessage(_ workout: CanovaWorkout) -> [UInt8] {
        var message = [UInt8]()
    ...
    private func createWorkoutStepMessage(_ step: CanovaWorkoutStep, index: Int, racePaceSeconds: Double) -> [UInt8] {
        var message = [UInt8]()
    ...
    private func mapDuration(_ step: CanovaWorkoutStep) -> (UInt8, UInt32) {
    ...
    private func mapTarget(_ step: CanovaWorkoutStep, racePaceSeconds: Double) -> (UInt8, UInt32) {
    ...
    private func mapIntensity(_ stepType: CanovaWorkoutStep.StepType) -> UInt8 {
        switch stepType {
