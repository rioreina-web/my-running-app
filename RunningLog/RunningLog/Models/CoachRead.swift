...
//    - `questions` (v3) is the 0-2 soft questions the coach leaves the
//      athlete to sit with. Rendered italic, set apart from the
//      paragraph. Rows written before the v3 column landed have no
//      `questions` key — the decoder treats a missing key as `[]`.
...
// MARK: - Decoding (tolerant of pre-v3 rows)

extension CoachRead {
    /// Hand-written so `questions` can be absent (rows generated by the
    /// v1/v2 prompt, or read before the `questions` column migration
    /// was pushed) and still decode as an empty array. Every other field
    /// keeps the strict behaviour the synthesized decoder had. Lives in
    /// an extension so the memberwise initializer stays available to
    /// previews and tests.
    init(from decoder: Decoder) throws {
        let c = try decoder.container(keyedBy: CodingKeys.self)
        id = try c.decode(UUID.self, forKey: .id)
        readDate = try c.decode(Date.self, forKey: .readDate)
        headline = try c.decode(String.self, forKey: .headline)
        paragraph = try c.decode([Segment].self, forKey: .paragraph)
        questions = try c.decodeIfPresent([String].self, forKey: .questions) ?? []
        cantSee = try c.decodeIfPresent(CantSee.self, forKey: .cantSee)
        sources = try c.decode(Sources.self, forKey: .sources)
