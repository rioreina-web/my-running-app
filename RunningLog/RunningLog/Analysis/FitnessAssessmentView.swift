...
struct FitnessAssessmentView: View {
    ...
    @State private var viewModel: FitnessAssessmentViewModel
    ...
    init(
        goalTimeSeconds: Int,
        raceDate: Date,
        currentWeeklyMileage: Double = 30,
        onComplete: @escaping (FitnessAssessment) -> Void
    ) {
        self.goalTimeSeconds = goalTimeSeconds
        self.raceDate = raceDate
        self.onComplete = onComplete
        _viewModel = State(wrappedValue: FitnessAssessmentViewModel(currentWeeklyMileage: currentWeeklyMileage))
    }
...
struct NavigationButtons: View {
    @Bindable var viewModel: FitnessAssessmentViewModel
    let onSkip: () -> Void
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // Recent 5K
                QuestionSection(title: "RECENT 5K TIME?") {
                    HStack(spacing: 12) {
                        SelectableChip(title: "No", isSelected: !viewModel.hasRecent5k) {
                            viewModel.hasRecent5k = false
                        }
                        SelectableChip(title: "Yes", isSelected: viewModel.hasRecent5k) {
                            viewModel.hasRecent5k = true
                        }
                    }

                    if viewModel.hasRecent5k {
                        TimeInputRow(
                            label: "5K Time",
                            hours: .constant(0),
                ...
                // Recent 10K
                QuestionSection(title: "RECENT 10K TIME?") {
                    HStack(spacing: 12) {
                        SelectableChip(title: "No", isSelected: !viewModel.hasRecent10k) {
                            viewModel.hasRecent10k = false
                        }
                        SelectableChip(title: "Yes", isSelected: viewModel.hasRecent10k) {
                            viewModel.hasRecent10k = true
                        }
                    }

                    if viewModel.hasRecent10k {
                        TimeInputRow(
                            label: "10K Time",
                            hours: .constant(0),
